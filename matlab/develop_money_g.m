...
DrawFormattedText(window, double('欢迎参加本测试！\n\n在本阶段，你将会看到六个颜色不同的圆形，\n\n你的任务是找到红色或绿色的圆形（每次只出现一种），\n\n如果该圆形内部的线段是竖着的，请用左手食指按键盘上的Z键；\n\n如果该圆形内部的线段是横着的，请用右手食指按键盘上的M键。\n\n每个正确反应之后，会得到一定的分数，\n\n最后得到的总分数可以用来兑换金钱。\n\n请在确保正确的前提下尽量快地做出反应。\n\n正式测试前会有30次练习，练习中不会得到分数。\n\n如果准备好了，请按任意键继续。'), 'center', 'center', white);
Screen('Flip', window);
KbStrokeWait;
...
[KeyIsDown,secs,KeyCode] = KbCheck;
    ...
    elseif KeyCode(mKey) && a > 0.5  && color(trial) == 'g'  &&  toc < 2
         respToBeMade = false;
         Screen('Flip', window);
         WaitSecs(1.0);
    ...
    elseif KeyCode(mKey) && a > 0.5  && color(trial) == 'r'  &&  toc < 2
         respToBeMade = false;
         Screen('Flip', window);
         WaitSecs(1.0);
         DrawFormattedText(window, double('正确'), 'center', 'center', white);
    ...
    elseif  KeyCode(zKey)  && a < 0.5  && color(trial) == 'g'  &&  toc < 2
         respToBeMade = false;
         Screen('Flip', window);
         WaitSecs(1.0);
         DrawFormattedText(window, double('正确'), 'center', 'center', white);
    ...
    elseif  KeyCode(zKey)  && a < 0.5  && color(trial) == 'r'  &&  toc < 2
         respToBeMade = false;
         Screen('Flip', window);
         WaitSecs(1.0);
         DrawFormattedText(window, double('正确'), 'center', 'center', white);
    ...
    elseif  KeyCode(mKey) && a < 0.5  || KeyCode(zKey)  && a > 0.5  &&  toc < 2
        respToBeMade = false;
        Screen('Flip', window);
        WaitSecs(1.0);
...
color = repmat(['r','g'],1,30);
color1 = randsample(color,60);
color2 = randsample(color,60);
color3 = randsample(color,60);
color = [color1,color2,color3];

position = repmat(1:6,1,10);
position1 = randsample(position,60);
position2 = randsample(position,60);
position3 = randsample(position,60);
...
[KeyIsDown,secs,KeyCode] = KbCheck;
    ...
    elseif KeyCode(mKey) && a > 0.5  && color(trial) == 'g'  &&  toc < 2
         ...
         Screen('Flip', window);
         WaitSecs(1.0);
         total = points(trial) + total;
         DrawFormattedText(window, [double(['+ ' num2str(points(trial)) ' ']) double(' \n\n ') double('总分')  double([' ' num2str(total) ' '])],  'center', 'center', white);
         Screen('Flip', window);
    WaitSecs(1.5);
    ...
    elseif KeyCode(mKey) && a > 0.5  && color(trial) == 'r'  &&  toc < 2
         ...
         Screen('Flip', window);
         WaitSecs(1.0);
         total = points_l + total;
         DrawFormattedText(window, [double(['+ ' num2str(points_l) ' ']) double(' \n\n ') double('总分')  double([' ' num2str(total) ' '])],  'center',  'center', white);
         Screen('Flip', window);
    WaitSecs(1.5);
...
% 第二阶段指导语
Screen('TextSize', window, 30);
DrawFormattedText(window, double('在本阶段，你将会看到六个颜色不同的形状，\n\n或者是五个圆形一个菱形，或者是五个菱形一个圆形\n\n你的任务是找到那个不同的形状，\n\n如果该形状内部的线段是竖着的，请用左手食指按键盘上的Z键；\n\n如果该形状内部的线段是横着的，请用右手食指按键盘上的M键。\n\n本阶段不会得到金钱奖励，而且不需要关注颜色。\n\n请在确保正确的前提下尽量快地做出反应。\n\n正式测试前会有18次练习。\n\n如果准备好了，请按任意键继续。'), 'center', 'center', white);
Screen('Flip', window);
KbStrokeWait;
...
color_test = repmat(['r','g','o','o'],1,15);
color_test1 = randsample(color_test,60);
color_test2 = randsample(color_test,60);
color_test3 = randsample(color_test,60);
color_test = [color_test1,color_test2,color_test3];

singleton = repmat(['c','d'],1,30);
singleton1 = randsample(singleton,60);
singleton2 = randsample(singleton,60);
singleton3 = randsample(singleton,60);
...
position_o = [p_o1,p_o2,p_o3];
position_d = [p_d1,p_d2,p_d3];
...
DrawFormattedText(window, double('全部测试结束\n\n谢谢参与!'), 'center', 'center', white);
Screen('Flip', window);
KbStrokeWait;

% 记录数据
results = [training,test];          
csvwrite(['develop_money_g' char(answer(1)) '.csv'],results,0);
